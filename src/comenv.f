***
     ...
     &     ffb1,ffb2,cinc,nov1,tsn1,tsn2,
     &     arem1,arem2,
* A. Tanikawa adds here for merger inflation 21/10/14
     &     tphys,tmrg1,tmrg2,tcon1,tcon2,rmrg1,rmrg2,
...
*     A. Tanikawa adds here 20/05/05
      ...
      real*8 tsn1, tsn2
      real*8 arem1,arem2
      real*8 tphys,tmrg1,tmrg2,tcon1,tcon2,rmrg1,rmrg2
* A. Tanikawa corrects reinitialization 21/08/27
      logical reinit
      integer catchsn,catchnr
      real*8 corfac
...
*
     ...
     &     R1,L1,KW1,MC1,RC1,MENV,RENV,K21,ffb1,tsn1,
     &     jspin1,arem1,
* A. Tanikawa adds here for merger inflation 21/10/14
     &     tphys,tmrg1,tcon1,rmrg1)
*
      OSPIN1 = JSPIN1/(K21*R1*R1*(M1-MC1)+K3*RC1*RC1*MC1)
      MENVD = MENV/(M1-MC1)
      RZAMS = RZAMSF(M01)
* A. Tanikawa modifies here
...
*
     ...
     &     R2,L2,KW2,MC2,RC2,MENV,RENV,K22,ffb2,tsn2,
     &     jspin2,arem2,
* A. Tanikawa adds here for merger inflation 21/10/14
     &     tphys,tmrg2,tcon2,rmrg2)
*
...
* A. Tanikawa changes here to treat HG as MS 21/08/07
!      IF(KW2.GE.2.AND.KW2.LE.9.AND.KW2.NE.7)THEN
      IF(KW2.GE.3.AND.KW2.LE.9.AND.KW2.NE.7)THEN
*
         MENVD = MENV/(M2-MC2)
         RZAMS = RZAMSF(M02)
* A. Tanikawa modifies here
...
!!         ebindi = ebindi+M2*(m2-mc2)/(lambda*r2)        
!      if(askInUseOrNot()) then
!         ebindi = ebindi+M2*(m2-mc2)/(lambda*r2)        
!      else
!         LAMB2 = CELAMF(KW,M02,L2,R2,RZAMS,MENVD,LAMBDA)
...
*
* A. Tanikawa restores here from Kinugawa prescription 21/02/23
!         IF(CEFLAG.NE.3) EORBI = M1*M2/(2.D0*SEP)
!      ELSE
!         IF(CEFLAG.NE.3) EORBI = M1*M2/(2.D0*SEP)
...
*
            M1 = MC1
* A. Tanikawa changes spin 21/08/30
            jspin1 = ospin1*k3*rc1*rc1*mc1
*
            CALL star(KW1,M01,M1,TM1,TN,TSCLS1,LUMS,GB,ZPARS)
            CALL hrdiag(M01,AJ1,M1,TM1,TN,TSCLS1,LUMS,GB,ZPARS,
     &           R1,L1,KW1,MC1,RC1,MENV,RENV,K21,ffb1,tsn1,
...
*
            IF(KW1.GE.13)THEN
* A. Tanikawa corrects reinitialization 21/08/27
               if(reinit .and. kw1.le.14) catchsn = 1
               if(reinit .and. kw1.eq.15) catchnr = 1
*
               CALL kick(KW1,MF,M1,M2,ECC,SEPF,JORB,ffb1,VS,cinc,nov1,
* A. Tanikawa corrects reinitialization 21/08/27
     &              delv,delj,dele)
*
...
* A. Tanikawa corrects reinitialization 21/08/27
               if(reinit .and. kw2.le.14) catchsn = 2
               if(reinit .and. kw2.eq.15) catchnr = 1
*
               CALL kick(KW2,MF,M2,M1,ECC,SEPF,JORB,ffb2,VS,cinc,nov1,
* A. Tanikawa corrects reinitialization 21/08/27
     &              delv,delj,dele)
*)
               IF(ECC.GT.1.D0) GOTO 30
...
*
         ...
         ENDIF
         CALL hrdiag(M01,AJ1,M1,TM1,TN,TSCLS1,LUMS,GB,ZPARS,
     &        R1,L1,KW,MC1,RC1,MENV,RENV,K21,ffb1,tsn1,
     &        jspin1,arem1,
* A. Tanikawa adds here for merger inflation 21/10/14
     &        tphys,tmrg2,tcon2,rmrg2)
*
         ...
         ECC = 0.D0
* A. Tanikawa corrects reinitialization 21/08/27
         if(reinit .and. (kw1.eq.13 .or. kw1.eq.14)) catchsn = 1
         if(reinit .and. (kw1.eq.15))                catchnr = 1
*
      ELSE
*
...
* A. Tanikawa corrects reinitialization 21/08/27
         if(reinit) then
            if(catchsn.ge.1) then
               corfac = jorb/sqrt(delj(1)**2+delj(2)**2+delj(3)**2)
               delj(1) = corfac*delj(1)
               delj(2) = corfac*delj(2)
               delj(3) = corfac*delj(3)
            else
               delj(1) = 0.
               delj(2) = 0.
               delj(3) = jorb
