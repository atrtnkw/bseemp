...
*
      logm = log10(mass)
* Tanikawa changes here 21/09/27
...
*
* Tanikawa changes here 22/01/27
!         logmt = log10(mt)
...
!         k2g = (F - 0.033d0*log10(lum) + 0.4d0*x)/(1.d0+y*(lbgb/lum)+x)
         ...
         F = 0.208d0 + 0.125d0*logmt - 0.035d0*logmt**2
         B = 1.0d+04*mttmp**(3.d0/2.d0)/(1.d0+0.1d0*mttmp**(3.d0/2.d0))
         x = ((lumtmp-lbgbtmp)/B)**2
         y = (F - 0.033d0*log10(lbgbtmp))/k2bgb - 1.d0
         k2g = (F - 0.033d0*log10(lumtmp) + 0.4d0*x)
     &        /(1.d0+y*(lbgbtmp/lumtmp)+x)
*
      elseif(kw.eq.9)then
*
...
* A. Tanikawa adds here 21/08/05 (Normally, switched off)
            k2e = getK2strOfBluePhase(mt,mc,rzams,rtms,rad,k2e)
*
         elseif(kw.eq.7)then
