...
*
      ...
      LOGICAL isave,iplot
* Tanikawa modifies here for wind 20/03/04
!      REAL*8 rl,mlwind,vrotf,corerd
!      EXTERNAL rl,mlwind,vrotf,corerd
      REAL*8 rl,mlwind,vrotf,corerd,mlwindenhanced
      EXTERNAL rl,mlwind,vrotf,corerd,mlwindenhanced
*
* Tanikawa modifies here for spin 20/03/03
      integer SizeOfbcm
      parameter(SizeOfbcm=50000)
      REAL bcm(SizeOfbcm,36),bpp(8000,18)
...
*
      ...
      REAL*8 pts3
      COMMON /POINTS/ pts3
      !real*8 UpperLimitMass, LowerLimitMass
      !parameter (UpperLimitMass=10000,LowerLimitMass=8)
      real*8 GravOverCele,GravConst
      parameter (GravOverCele=30.12,GravConst=3.916d8)
!      real*8 GravConstRsunMsunDay
!      parameter (GravConstRsunMsunDay=2.935d3)
      real*8 mred,fmu,nindex,dnindex
      real*8 rconv
!      logical NewDynTide
!      logical NewMassTransfer
!      parameter (NewDynTide=.false.,NewMassTransfer=.false.)
!      parameter (NewDynTide=.true.,NewMassTransfer=.true.)
      real*8 tmrg(2), tcon(2), tkhq(2), rmrg(2), emrg(2)
* A. Tanikawa changes this for spin evolution after BH formation
      logical spinevol,hypedd,hypeddflag
      parameter(spinevol=.true.,hypedd=.false.)
      real*8 hypeddfac
      parameter(hypeddfac=1.0d4)
*
* A. Tanikawa corrects reinitialization 21/08/27
      logical reinit
      real*8 trsrt
...
*
     ...
     &        rm,lum,kstar(k),mc,rc,me,re,k2,ffb(k),typesn(k),
     &        jspin(k),arem(k),
* A. Tanikawa adds here for merger inflation 21/10/14
     &        tphys,tmrg(k),tcon(k),rmrg(k))
*
         aj(k) = age
         epoch(k) = tphys - age
         rad(k) = rm
...
* A. Tanikawa corrects reinitialization 21/08/27
         if (reinit .and. rm.gt.rol(k)) then
            radx(k) = Max(rc, rol(k))
...
* A. Tanikawa corrects reinitialization 21/08/27
      if(reinit) then
         ...
         dele(2)  = 0.
         dele(3)  = 0.
         if(kstar(1).eq.15) dtmi(1) = 1e10
         if(kstar(2).eq.15) dtmi(2) = 1e10
         coel = .false.
         com  = .false.
      else
         ip = 0
...
*
 4    iter = 0
      ...
      bsymb = .false.
      esymb = .false.
* A. Tanikawa allows hyper-Eddington accretion. 22/01/18
      hypeddflag = .false.
*
      tphys0 = tphys
      ecc1 = ecc
...
*
     ...
     &                 massc(k),rlperi,zwind)
                  if(WindEnhanced) then
                     dmr(k) = mlwindenhanced(kstar(k),lumin(k),rad(k),
     &                    mass(k),massc(k),ospin(k),dmr(k))
                  endif
               else
                  dmr(k) = mlwind(kstar(k),lumin(k),rad(k),mass(k),
     &                 massc(k),rlperi,z)
...
* Tanikawa adds here
               if(askInUseOrNot()) then
                  dmt(3-k) = min(dmt(3-k),2.08d-03*eddfac
     &                 *(1.d0/(1.d0 + zpars(11)))*rad(3-k))
               endif
*
            else
               dmr(k) = 0.d0
...
*
     ...
     &         (lacc.lt.0.01d0.and.bsymb))then
               jp = MIN(8000,jp + 1)
* A. Tanikawa corrects reinitialization 21/08/27
               if(reinit .and. bpp(jp-1,10).eq.10.0) then
                  jp = jp - 1
               endif
*
               bpp(jp,1) = tphys
...
* A. Tanikawa adds this for spin 20/03/03 + 20/05/06+ 21/08/03
* A. Tanikawa changes this for spin evolution after BH formation
               if(spinevol) then
                  bpp(jp,11) = jspin(1)/(GravOverCele*mass(1)**2)
                  bpp(jp,12) = jspin(2)/(GravOverCele*mass(2)**2)
               else
...
*
               ...
               bpp(jp,18) = bcm(ip,21)
* A. Tanikawa corrects reinitialization 21/08/27
               if(reinit .and. tphys.eq.trsrt) then
                  jp = jp - 1
               endif
*
*
...
* A. Tanikawa assumes BH wind accretion forms disk. 22/01/15
            if(spinevol .and. kstar(k).eq.14) then
               djt = dmt(k)*twopi*aursun*sqrt(aursun*mass(k)*rad(k))
               djorb = djorb - (djtx(k) - djt) ! disk angular momentum goes back to orbital angular momentum
               delet = delet - 0.              ! A. Tanikawa isn't sure if decc=0.
               djtx(k) = djt
            endif
*
...
* A. Tanikawa modifies here 20/05/26
!               if(askInUseOrNot()) then
               if(askInUseOrNot() .and. z.le.0.0001) then
*
                  djmb = 0.
               else
...
*
               if(askInUseOrNot()) then
                  if((kstar(k).eq.1.and.mass(k).ge.1.25d0)
* Tanikawa debugs this 20/05/21
!     &                 .or.askRadiativeOrNot(kstar(k),aj(k),mass(k))
     &                 .or.askRadiativeOrNot2(kstar(k),lumin(k),rad(k))
*
     &                 .or.kstar(k).eq.7)then
*     
* Radiative damping (Zahn, 1977, A&A, 57, 383 and 1975, A&A, 41, 329).
*     
* A. Tanikawa modifies here 20/07/16 Kinugawa+2020
                  if(.not. NewDynTide)then
                     tc = 1.592d-09*(mass(k)**2.84d0)
                  else
                     if(kstar(k).lt.7)then
                        rconv = 
     &                       getConvectiveCoreRadiusOfBluePhase
     &                       (mass(k))
                        tc = 10**(-0.42)*(rconv/rad(k))**7.5
                     else
                        tc = 10**(-0.93)*(0.5/rad(k))**6.7
                     endif
                  endif
*
                     f = 1.9782d+04
     &                    *SQRT((mass(k)*rad(k)*rad(k))/sep**5)*
...
*     
                     tc = mr23yr*(menv(k)*renv(k)*(rad(k)
     &                    -0.5d0*renv(k))/(3.d0*lumin(k)))**(1.d0/3.d0)
                     ttid = twopi/(1.0d-10 + ABS(oorb - ospin(k)))
                     f = MIN(1.d0,(ttid/(2.d0*tc)**2))
!                     f = MIN(1.d0,(ttid/(2.d0*tc))**2)
                     tcqr = 2.d0*f*q(3-k)*raa6
     &                    *menv(k)/(21.d0*tc*mass(k))
...
* Radiative damping (Zahn, 1977, A&A, 57, 383 and 1975, A&A, 41, 329).
*     
* A. Tanikawa modifies here 21/08/07 Kinugawa+2020
                     if(.not. NewDynTide)then
                        tc = 1.592d-09*(mass(k)**2.84d0)
                     else
                        if(kstar(k).lt.7)then
...
*
      do 505 , k = kmin,kmax
*
* A. Tanikawa adds here 22/01/15
         if(spinevol .and. kstar(k).eq.14) cycle
*
...
* A. Tanikawa modifies here for BH spin evolution 22/01/15
!         if(jspin(k).gt.jspbru.and.ABS(dtm).gt.tiny)then
         if(jspin(k).gt.jspbru.and.ABS(dtm).gt.tiny .and.
     &        (.not. (spinevol .and. kstar(k).eq.14)))then
*
            mew = 1.d0
...
! hogea
!         if(k.eq.1 .and. kstar(k).lt.13) then
!            write(*,'(a3,f10.4,i3,7e12.4)')
!     &           'hga',
!     &           tphys,kstar(k),mass(k),rad(k),
!     &           k2str(k),ospin(k),jspin(k),
!     &           jspin(k)/(GravOverCele*mass(k)**2),
!     &           jspbru/(GravOverCele*mass(k)**2)
!         endif
! hogea
*
...
* A. Tanikawa corrects reinitialization 21/08/27
      if(reinit .and. 
     &     catchsn+catchce+catchmg+catchnr+catchbd.eq.0) then
         msec(1)  = mass(1)
         dmsec(1) = -dms(1)/dt
         msec(2)  = mass(2)
         dmsec(2) = -dms(2)/dt
...
!               WRITE(0,*)' MASS EXCEEDED ',mass1i,mass2i,tbi,ecci
!* A. Tanikawa corrects reinitialization 21/08/27
!               if(reinit) catcherr = 1
!*
!               goto 140
!            endif
* A. Tanikawa adds here 20/05/26
* A. Tanikawa changes here 20/12/23
!!            if(mt.lt.LowerLimitMass .and. kw .lt. 7)then ! desirable for NewMassTransfer
!            if(m0.lt.LowerLimitMass .and. kw .lt. 7)then
!*
!               WRITE(99,*)' mass falled ',mass1i,mass2i,tbi,ecci
!               write(99,*)' mass falled ',tphys,k,kw,m0,mt,mc
!* A. Tanikawa corrects reinitialization 21/08/27
!               if(reinit) catcherr = 1
!*
!               goto 140
!            endif
*
!            if(m0.lt.LowerLimitMass)then
!               WRITE(99,*)' mass below ',mass1i,mass2i,tbi,ecci,m0,
!     &              kw
!               goto 140
!            endif
         else
* A. Tanikawa changes here 21/02/19
!            if(mt.gt.100.d0)then
            if(mt.gt.200.d0)then
*
...
*
     ...
     &        rm,lum,kw,mc,rc,me,re,k2,ffb(k),typesn(k),
     &        jspin(k),arem(k),
* A. Tanikawa adds here for merger inflation 21/10/14
     &        tphys,tmrg(k),tcon(k),rmrg(k))
*
...
*
            if(sgl)then
               CALL kick(kw,mass(k),mt,0.d0,0.d0,-1.d0,0.d0,ffb(k),vs,
     &              cinc,nov1,
* A. Tanikawa corrects reinitialization 21/08/27
     &              delv,delj,dele)
*
            else
               CALL kick(kw,mass(k),mt,mass(3-k),ecc,sep,jorb,ffb(k),vs,
     &              cinc,nov1,
* A. Tanikawa corrects reinitialization 21/08/27
     &              delv,delj,dele)
*)
...
* A. Tanikawa changes this for spin evolution after BH formation
               if(spinevol) then
                  jspin(k) = arem(k)*(GravOverCele*mc**2)
...
!         if(askInUseOrNot()) then
         if(askInUseOrNot() .and. askInScopeOfApplication(m0)) then ! mt->m0 (21/09/24)
*
...
*
         if(snova)then
            ...
            dtm = 0.d0
* A. Tanikawa corrects reinitialization 21/08/27
            if(reinit .and. catchsn.ge.1) tphysf = tphys
*
            goto 4
         endif
      endif
*
     ...
     &      or.tphys.lt.tiny)then
            ip = ip + 1
* A. Tanikawa add here 21/02/21
            if(ip.ge.SizeOfbcm-1)ip=SizeOfbcm-1
*
            bcm(ip,1) = tphys
     ...
     &                       (rad(1)**2.d0))**(1.d0/4.d0))
            ...
            bcm(ip,11) = renv(1)
            bcm(ip,12) = epoch(1)
            bcm(ip,13) = jspin(1)/(GravOverCele*mass(1)**2) !ospin(1)
     ...
     &                       (rad(2)**2.d0))**(1.d0/4.d0))
            ...
            bcm(ip,25) = renv(2)
            bcm(ip,26) = epoch(2)
            bcm(ip,27) = jspin(2)/(GravOverCele*mass(2)**2) ! ospin(2)
...
!         if(askInUseOrNot() .and.
!     &        askInScopeOfApplication(mass0(j1))) then
!            if(intpol.gt.0 .and.
!     &           tphys.gt.tphys00 .and. rad(j1).lt.rol(j1)) then
!               intpol = 0
!               write(0,*)'Caution: MT stops.',mass1i,mass2i,tbi,ecci
!            endif
!         endif
*
         if(intpol.gt.0)then
...
!         goto 140
!      endif
      if((tphys+dtm).ge.tphys00 .and. dtm.eq.1.0d-10)then
         WRITE(0,*)' TOO SMALL DTM ',mass1i,mass2i,tbi,ecci,dtm
         goto 140
      endif
      goto 5
*
...
* A. Tanikawa corrects reinitialization 21/08/27
      if(reinit .and. tphys.ge.tphysf) goto 140
*
...
*
 8    dme = 2.08d-03*eddfac*(1.d0/(1.d0 + zpars(11)))*rad(j2)*tb ! dme is correct for BHs, but for WDs/NSs?...
      supedd = .false.
      novae = .false.
      disk = .false.
*
...
*
      do 13 , k = 1,2
         if(askInUseOrNot()) then
            tkh(k) = 3.12d+07*mass(k)/(rad(k)*lumin(k))
         else
...
*
      if(askInUseOrNot()) then
         tdyn = 5.61d-05*SQRT(rad(j1)**3/mass(j1))
      else
...
* Tanikawa modeifies here
     ...
     &        .or.
     &        (.not.askInUseOrNot()
     &        .and.((ABS(ABS(2*kstar(j1)-11)-3).eq.2.or.kstar(j1).eq.9).
...
* A. Tanikawa adds for merger inflation here 21/10/14
...
*
     ...
     &        ffb(j1),ffb(j2),cinc,nov1,typesn(j1),typesn(j2),
     &        arem(j1),arem(j2),
* A. Tanikawa adds here for merger inflation 21/10/14
     &        tphys,tmrg(j1),tmrg(j2),
     &        tcon(j1),tcon(j2),rmrg(j1),rmrg(j2),
*
* A. Tanikawa corrects reinitialization 21/08/27
     &        reinit,catchsn,catchnr,delv,delj,dele)
         if(reinit) then
            catchce = 1
            catchmg = 0
            if(coel) catchmg = 1
            if(ecc.gt.1.) catchbd = 1
            if(catchsn.eq.1) then
               catchsn = j1
            elseif(catchsn.eq.2) then
               catchsn = j2
            endif
            tphysf  = tphys
         endif
...
!         if(eddfac.lt.10.d0)then
         if(eddfac.lt.10.d0 .or. kstar(j2).eq.13 
     &        .or. kstar(j2).eq.14)then
*
            dm2 = MIN(dme*taum/tb,dm1)
            if(dm2.lt.dm1) supedd = .true. 
         else
...
!            mass(j2) = 0.d0
!         endif
         if(kstar(j2).eq.10.and.mass(j2).gt.mch)then         
            kstar(j2) = 15
            mass(j2) = 0.d0
         elseif(kstar(j2).eq.11.and.dm1.gt.0.8)then
* Violent merger criterion (Sato et al. 2016, 821, 67)
            kstar(j2) = 15
            mass(j2) = 0.d0
         elseif(kstar(j2).eq.11.and.mass(j2).gt.mch)then
* Converting into ONe WD (Saio, Nomoto 1985; Schwab et al. 16, MNRAS, 463, 3461)
            kstar(j2) = 12
         endif
*
         coel = .true.
...
*
         if(.false.) then
            ...
            call bhmerger(mass,arem,cinc,
     &           kstar(j1),kstar(j2),mass(j1),mass(j2),
     &           arem(j1),arem(j2),cinc(j1),cinc(j2))
         endif
         coel = .true.
* A. Tanikawa corrects reinitialization 21/08/27
         if(reinit) catchmg = 1
...
*
* A. Tanikawa modifies here 20/07/16 Kinugawa+2020
* A. Tanikawa applies to the original SSE/BSE 21/02/20
!         if(askInUseOrNot()) then
         if(.not. NewMassTransfer) then
            dm1 = 3.0d-06*tb*(LOG(rad(j1)/rol(j1))**3)*
     &           MIN(mass(j1),5.d0)**2
!     if(j1.eq.1)write(*,*)tphys,kstar(1),dm1,rad(1),mass(1)
            if(kstar(j1).eq.2)then
...
*     dm1 = dm1*1.0d+03/MAX(rad(j1),1.0d-04)
            ...
            if(askBlueOrRed2(lumin(j1),rad(j1))) then
               nindex  = 3.
               dnindex = 0.0364
            else
               nindex  = 1.5
            ...
            mred = mass(j1)/(mass(j1)+mass(j2))
            fmu  = 4.*mred**1.5*(1.-mred)**0.5
     &           /(mred**0.5+(1.-mred)**0.5)**4
     &           *(sep/rad(j1))**3
            dm1  = tb*fmu*mass(j1)
!     &              /(rad(j1)**3/(GravConst*mass(j1)))**0.5
     &           /tdyn
     &           *((rad(j1)-rol(j1))/rad(j1))**(nindex+1.5)*dnindex
...
*
* A. Tanikawa changes here for non-conservative MT 21/02/25
!            dm2 = MIN(1.d0,10.d0*taum/tkh(j2))*dm1
            dm2 = MIN(betaacc,10.d0*taum/tkh(j2))*dm1
*
...
* A. Tanikawa changes here for non-conservative MT 21/03/04
!               dm2 = dm1
               dm2 = betaacc*dm1
*
...
!            dm2 = MIN(betaacc*dm1,dme)
            if(hypedd) then
               if(dm1.ge.hypeddfac*dme
     &              .and. kstar(j2).eq.14) then
                  dm2 = dm1
                  hypeddflag = .true.
               else
                  dm2 = MIN(dm1,dme)
...
!     &                   xi*dmt(k)*radx(3-k)*radx(3-k)*ospin(3-k))
            if(spinevol .and. kstar(k).eq.14) then
               djt = dmt(k)*twopi*aursun*sqrt(aursun*mass(k)*rad(k))
               djspint(k) =
     &              (2.d0/3.d0)*dmr(k)*radx(k)*radx(k)*ospin(k) - djt
               djt = djt - (2.d0/3.d0)*xi*dmt(k)
     &              *radx(3-k)*radx(3-k)*ospin(3-k)
               djorb = djorb + djt*dt
            else
               djspint(k) = (2.d0/3.d0)*(dmr(k)*radx(k)*radx(k)*ospin(k)
     &              - xi*dmt(k)*radx(3-k)*radx(3-k)*ospin(3-k))
...
* A. Tanikawa allows hyper-Eddington accretion. 22/01/18
!         if(disk)then
         if(hypedd .and. hypeddflag) then
            hypeddflag = .false.
            djt = dm2*twopi*aursun*SQRT(aursun*mass(j2)*radx(j2)) 
            djspint(j2) = djspint(j2) - djt 
            djorb = djorb + djt !+ (dm1-dm2)*radx(j1)*radx(j1)*ospin(j1) ! comment out to be the same as below 22/01/19
         else if(disk)then
*
*
...
* A. Tanikawa allows hyper-Eddington accretion. 22/01/18
         if(hypeddflag) then
            write(0,*)'Fatal error in evolv2.f related to hypedd'
            stop
         endif
*
         djtx(2) = djt
...
*     
                     f = 1.9782d+04*
     &                    SQRT((mass(k)*radx(k)*radx(k))/sep**5)*
     &                    tc*(1.d0+q(3-k))**(5.d0/6.d0)
                     tcqr = f*q(3-k)*raa6
                     rg2 = k2str(k)
                  elseif(kstar(k).le.9)then
                     ...
                     tc = mr23yr*(menv(k)*renv(k)*
     &                    (radx(k)-0.5d0*renv(k))/
     &                    (3.d0*lumin(k)))**(1.d0/3.d0)
                     ttid = twopi/(1.0d-10 + ABS(oorb - ospin(k)))
                     f = MIN(1.d0,(ttid/(2.d0*tc)**2))
                     tcqr = 2.d0*f*q(3-k)*raa6*menv(k)
     &                    /(21.d0*tc*mass(k))
                     rg2 = (k2str(k)*(mass(k)-massc(k)))/mass(k)
                  else
                     f = 7.33d-09*(lumin(k)/mass(k))**(5.d0/7.d0)
...
* A. Tanikawa corrects reinitialization 21/08/27
     ...
     &        catchsn+catchce+catchmg+catchnr+catchbd.eq.0) then
            msec(j1)  = mass(j1)
            dmsec(j1) = - (dm1 + dms(j1)) / dt
            msec(j2)  = mass(j2)
            dmsec(j2) = (dm22 - dms(j2)) /dt
            jsec      = jorb
            djsec     = -djorb/dt
            esec      = ecc
            desec     = -delet/dt
...
!               goto 140
!            endif
* A. Tanikawa adds here 20/05/26 and 20/12/23
            kw = kstar(k)
* A. Tanikawa changes here 20/12/23
!!            if(mt.lt.LowerLimitMass .and. kw .lt. 7)then ! desirable for NewMassTransfer
!            if(m0.lt.LowerLimitMass .and. kw .lt. 7)then
...
*
      ...
      bpp(jp,18) = bcm(ip,21)
*
* A. Tanikawa adds this 20/12/25
      if(jp.ge.8000) goto 140
*
*
* A. Tanikawa changes here to prevent HGCE 21/02/22
!      if(kstar(j1).ge.2.and.kstar(j1).le.9.and.kstar(j1).ne.7)then
...
*
      sgl = .true.
* A. Tanikawa corrects reinitialization 21/08/27
      if(reinit .and. (catchsn.ge.1 .or. catchmg.eq.1)) tphysf = tphys
*
...
* A. Tanikawa adds this 20/12/25
            if(jp.ge.8000) goto 140
*
* A. Tanikawa adds here 21/02/22
!            if((mass0(1).lt.LowerLimitMass .and. kstar(1).ne.6)
!     &           .or. (mass0(2).lt.LowerLimitMass .and. kstar(2).ne.6))
!     &           then
!               goto 140
!            endif
*
...
*
* A. Tanikawa adds this 20/12/25
* A. Tanikawa comments out this 21/02/25
!         if(jp.ge.8000) goto 140
*
      endif
*
...
* A. Tanikawa corrects reinitialization 21/08/27
         if(.not. reinit) then
            WRITE(0,*)' EVOLV2 ARRAY WARNING ',
     &           mass1i,mass2i,tbi,ecci,jp
            endif
*
      endif
      bcm(ip+1,1) = -1.0
